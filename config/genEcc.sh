...
function createCertificateTls {
 ...
 # OK: prime256v1  secp384r1
 # KKO: sect571r1 secp521r1 sect571k1
 openssl ecparam -name secp384r1 -genkey  -out $TARGET_DIR/$PRIV_DIR/$SERVER_KEY_FILENAME.pem
 ...
 openssl req -new -sha256  -key $TARGET_DIR/$PRIV_DIR/$SERVER_KEY_FILENAME.pem -out $TARGET_DIR/$PRIV_DIR/$SERVER_FILENAME.csr -subj $CERT_SUBJ
...
function printCsr {
   ...
   openssl req   -noout -text -in $TARGET_DIR/$PRIV_DIR/$SERVER_FILENAME.csr
   echo "# ### ############################################### ### #"
   echo ""
}
...
CA_SUBJ="$COMMON_SUBJ/CN=JmoECCCARoot/emailAddress=jmorille@gmail.com"
